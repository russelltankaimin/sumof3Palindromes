...
def main_algorithm(number,base):
    ...
    if length>=7:
        if types=="A1" or types=="A2" or types=="A3" or types=="A4":
            if odd:
                res=algorithm_1(number,config,base)
            elif not special:
                res=algorithm_2(number,config,base)
            else:
...
def algorithm_1(number,config,base):
    debug("Algorithm _ 1")
    rev_num=number[::-1]
    num=number
    length=len(num)
...
def algorithm_2(number,config,base):
    num=number[::-1]
    length=len(num)
    m=length>>1
    x,y,z=config[0],config[1],config[2]
