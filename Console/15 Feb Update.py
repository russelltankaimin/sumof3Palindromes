...
def main():
    ...
    if length>=7:
        ...
        if types=="A1" or types=="A2" or types=="A3" or types=="A4":
            if length%2 !=0:
                algorithm_1(x,y,z,base,number,1)
                return 0
            elif is_special(number,types)==False:
                algorithm_2(x,y,z,base,number,1)
                return 0
            else:
                algorithm_5(x,y,z,base,number,1)
                return 0
            ...
            if length%2!=0:
                algorithm_3(x,y,z,base,number,1)
                return 0
            elif is_special(number,types)==False:
...
def is_special(num_str,types):
  ...
  if length_of_p1%2!=0:
      ...
      #num=num_str[::-1]
      m=length_of_p1>>1
      if int(num_str[m])==0 or int(num_str[m-1])==0:
          return True
      else:
          return False
...
def algorithm_1(p1,p2,p3,base,num_string,val):
    print("Algorithm I")
    num=num_string[::-1]
...
def algorithm_2(p1,p2,p3,base,num_string,val=1):
    ...
    p3=[0]+p3
    carry.append((p1[1]+p2[1]+p3[1])//base)
    p1.append(d(int(num[2*m-2])-p2[1],base) if p3[1]<=int(num[2*m-3])-1 else d(int(num[2*m-2])-p2[1]-1,base))
    p2.append(d(int(num[2*m-3])-p3[1]-1,base))
    p3.append(d(int(num[1])-p1[2]-p2[2]-carry[1],base))
    carry.append((p1[2]+p2[2]+p3[2]+carry[1]-int(num[1]))//base)
    for i in range(3,m):
        p1.append(1 if p3[i-1]<=int(num[2*m-i-1])-1 else 0)
        p2.append(d(int(num[2*m-i-1])-p3[i-1]-1,base))
        p3.append(d(int(num[i-1])-p1[i]-p2[i]-carry[i-1],base))
        carry.append((p1[i]+p2[i]+p3[i]+carry[i-1]-int(num[i-1]))//base)
    p1.append(0)
    p2.append(d(int(num[m-1])-p3[m-1]-carry[m-1],base))
    carry.append((p1[m]+p2[m]+p3[m-1]+carry[m-1]-int(num[m-1]))//base)
    c=carry[m]
    if c==1:
...
def algorithm_3(p1,p2,p3,base,num_string,val):
    print("Algorithm III")
    num=num_string[::-1]
    p1.remove(p1[0])
    length=len(num)
...
def algorithm_5(p1,p2,p3,base,num_string,val):
    print("Algorithm V")
    half=len(num_string)>>1
    s=[0]*(len(num_string))
    s[half]=1
    s[half-1]=1
    n_prime=big_sub(num_string,s,base)
    t_prime=n_prime
    if t_prime[half]==0 or t_prime[half-1]==0:
        s[half]=2
        s[half-1]=2
        n_prime=big_sub(num_string,s,base)
    state=check_type(n_prime,base) #I copied off the top, else very lazy
    if state=="A1":
        p1,p2,p3=construct_A1(n_prime,base)
    elif state=="A2":
        p1,p2,p3=construct_A2(n_prime,base)
    elif state=="A3":
        p1,p2,p3=construct_A3(n_prime,base)
    elif state=="A4":
        p1,p2,p3=construct_A4(n_prime,base)
    elif state=="A5":
        p1,p2,p3=construct_A5(n_prime,base)
    elif state=="A6":
        p1,p2,p3=construct_A6(n_prime,base)
    elif state=="B1":
        p1,p2,p3=construct_B1(n_prime,base)            
    elif state=="B2":
        p1,p2,p3=construct_B2(n_prime,base)    
    elif state=="B3":
        p1,p2,p3=construct_B3(n_prime,base) 
    elif state=="B4":
        p1,p2,p3=construct_B4(n_prime,base)
    elif state=="B5":
        p1,p2,p3=construct_B5(n_prime,base)
    elif state=="B6":
        p1,p2,p3=construct_B6(n_prime,base)
    elif state=="B7":
        p1,p2,p3=construct_B7(n_prime,base)        
    actual_length=len(n_prime)
    if state=="A5" or state=="A6":
        length_of_p1=actual_length-1
    else:
        length_of_p1=actual_length
    if length_of_p1 %2==0:
        if state[0]=="A":
            p1,p2,p3=algorithm_2(p1,p2,p3,base,n_prime,2)
        else:
            p1,p2,p3=algorithm_4(p1,p2,p3,base,n_prime,2)
    else:
        z1=d(int(n_prime[-1])-int(n_prime[2]),base)
        if int(n_prime[0])==1 and int(n_prime[1])<=2 and z1!=0:
